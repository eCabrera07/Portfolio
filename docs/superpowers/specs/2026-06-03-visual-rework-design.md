...
```css
height: 1px;
background: linear-gradient(to right, transparent, oklch(0.72 0.22 295 / 0.3), oklch(0.68 0.2 220 / 0.2), transparent);
```
...
### Section heading
```jsx
<h2>Selected Engineering <em className="italic text-gradient-aurora">Work</em></h2>
```
  ...
  <div key={group.label} className="flex items-baseline gap-3 flex-wrap">
    ...
    <div className="flex gap-2 flex-wrap">
      {group.items.map((item) => (
        <span key={item} className={`badge badge-${group.color}`}>{item}</span>
      ))}
    </div>
  </div>
))}
...
### Section heading
```jsx
<h2>Skills recruiters can <em className="italic text-gradient-aurora">scan quickly</em></h2>
```
...
### Heading
```jsx
<h2>Let&apos;s build something <em className="italic text-gradient-aurora">useful.</em></h2>
```
